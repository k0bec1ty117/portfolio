...
#モデルの準備(Encoder)
file_path_en = 'intel/driver-action-recognition-adas-0002/driver-action-recognition-adas-0002-encoder/FP32/driver-action-recognition-adas-0002-encoder'
model_en= file_path_en + '.xml'
weights_en = file_path_en + '.bin'

#モデルの準備(Decoder)
file_path_de = 'intel/driver-action-recognition-adas-0002/driver-action-recognition-adas-0002-decoder/FP32/driver-action-recognition-adas-0002-decoder'
...
# カメラ準備 
cap = cv2.VideoCapture("image/driving.mp4")

# ラベル準備
f = open('others/driver_actions.txt', 'r')
...
while True:
    ...
    if len(encoder_output) == sample_duration:
        ...
        probs = np.squeeze(exp / np.sum(exp, axis=None))

        # 出力値が最大のインデックスを得る 
        index_max = np.argmax(probs)
        probs = '{:.2f}'.format(probs[index_max] * 100) 
